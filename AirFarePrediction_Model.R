...
airlines_dom <- read.csv("C:/EXCELR/DataSciencePoject/Project_ Air Fare prediction/FinalSubmissionProject/dataset_for_Modelling_domestic.csv")
...
airlines_dom_seq <- seq(as.Date("2018-04-01"), as.Date("2019-06-10"), by = "day") # Defining sequence
length(airlines_dom_seq)
class(airlines_dom_seq)
str(airlines_dom_seq)
...
ts_info(airlines_dom_ts)

View(airlines_dom_ts)

plot(airlines_dom_ts, main="Time series plot,Time Vs Net Fare", ylab="Net Fare", xlab="Time")
points(airlines_dom_ts, cex = .5, col = "dark red")

abline(reg=lm(airlines_dom_ts~time(airlines_dom_ts)))

start(airlines_dom_ts)
end(airlines_dom_ts)
frequency(airlines_dom_ts)
...
split_airlines <- ts_split(ts.obj = airlines_dom_ts, sample.out = 30)
...
airlines_dom_sma <- SMA(airlines_dom_ts, n=10) # n= 1,2 ,3, 4,5,6,......
plot.ts(airlines_dom_sma)
...
plot(log(airlines_dom_ts))

# Making the data stationary(homoogenizing mean)
#It will make the mean equal
plot(diff(log(airlines_dom_ts)))

acf(airlines_dom_ts)
acf(diff(log(airlines_dom_ts))) # determines the value of q
# q= 1
...
mstsNetFare <- msts(airlines_dom$AvgNetFare,seasonal.periods = c(7,365.25),start = decimal_date(as.Date("2018-04-01")),end=decimal_date(as.Date("2019-06-10")))
start(mstsNetFare)
end(mstsNetFare)
str(mstsNetFare)
class(mstsNetFare)
...
set.seed(123)
neural_net_Model2 <- nnetar(training_airlines, decay=0.075, maxit=100, lambda=0.5)
...
set.seed(123)
neural_net_Model3 <- nnetar(training_airlines, decay=0.075, maxit=100, lambda=0.5, scale.inputs = TRUE)
...
new_model <- HoltWinters(airlines_dom_ts,gamma = FALSE)
new_model
new_model$seasonal
